...
**API ключ Grok:**
- `[REDACTED — хранится в /opt/grok-news/.env на VPS]`
- На сервере хранить в `/opt/grok-news/.env` как `GROK_API_KEY=...`
- Claude Code должен: 1) открыть vault.myserver-ai.ru через браузер, 2) найти ключ, 3) записать в .env — **или** спросить пользователя вставить ключ вручную

**Nginx:** добавить location `/grok-news/` → `http://127.0.0.1:8770/`

**Фронтенд:**
- Если Homepage: через `custom.js` или iframe на отдельную страницу `/grok-news/ui`
