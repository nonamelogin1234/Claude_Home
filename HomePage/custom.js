/* ═══════════════════════════════════════════
   ...
   Fixed for Homepage v1.12.3 real DOM structure
═══════════════════════════════════════════ */
...
function buildWidgets(container) {
  ...
  var newsCard = buildNews();

  if (servicesEl && servicesEl.parentNode) {
    var parent = servicesEl.parentNode;
    parent.insertBefore(newsCard, servicesEl);
    parent.insertBefore(row, newsCard);
  } else {
...
function init() {
  ...
  ['information-widgets', 'information-widgets-right', 'widgets-wrap'].forEach(function(id) {
    var node = document.getElementById(id);
    if (node) node.style.display = 'none';
  });

  var servicesEl = document.getElementById('services');
  var container = servicesEl ? servicesEl.parentNode : document.querySelector('main') || document.body;
