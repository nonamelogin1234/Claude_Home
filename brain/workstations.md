...
| `telegram` | 🟢 Работает | `mcp-telegram` запускается через `scripts/telegram-mcp/mcp-telegram-wrapper.mjs`; конфиги `C:\Users\no-na\.codex\config.toml` и `C:\Users\no-na\.claude\settings.json`; включён `MCP_TELEGRAM_READONLY=1` |
