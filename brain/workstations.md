...
| `telegram` | 🟡 Настроен, но не авторизован | `scripts/telegram-mcp/mcp-telegram.cmd` (тот же код, что на домашнем ПК). Сессии `.telegram-agent` нет — нужен отдельный интерактивный логин (my.telegram.org или импорт tdata), не сделано. |
