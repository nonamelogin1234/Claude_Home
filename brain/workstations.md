...
- 2026-05-28: подключён официальный Notion MCP для Codex Desktop на рабочем ПК. Конфиг: `C:\Users\gor-r\.codex\config.toml`, блок `[mcp_servers.notion] url = "https://mcp.notion.com/mcp"`. Авторизация выполнена через OAuth командой `C:\Users\gor-r\AppData\Local\OpenAI\Codex\bin\958d608b5e0546a5\codex.exe mcp login notion`; `codex mcp list` показывает `notion enabled OAuth`. Для появления инструментов Notion в уже открытой сессии может понадобиться перезапуск Codex/новый чат.
- 2026-05-28: точка входа Notion для Codex — страница `Домашняя страница`, URL `https://www.notion.so/36ea682f4234801c8396c21a0bf1b5de`, page id `36ea682f-4234-801c-8396-c21a0bf1b5de`. В новых чатах не пытаться перечислять заметки поиском по слову "заметки": сначала делать `notion_fetch` этой страницы, затем читать дочерние страницы из ссылок в контенте.
- 2026-05-28: известные дочерние страницы Notion: `Не забыть!` (`https://www.notion.so/36ea682f423480c08496fce4de784802`) и `Сотрудники` (`https://www.notion.so/36ea682f423480038c1cc6b24024b505`). Официальный Notion search не гарантирует полный обход workspace; для надежной работы держать нужные заметки под корневой страницей или в базе/индексе, доступной через эту точку входа.
- 2026-05-28: добавлен локальный MCP-адаптер `notion_home` поверх Notion API. Код: `scripts/notion_home_mcp.py`; запуск: `C:\Users\gor-r\.codex\scripts\notion-home-mcp.ps1`; конфиг Codex: `C:\Users\gor-r\.codex\config.toml`, блок `[mcp_servers.notion_home]`. Инструменты адаптера должны стартовать от корневой страницы `Домашняя страница`, а не от глобального Notion search: `fetch_home`, `list_tree`, `fetch_page`, `fetch_page_by_title`, `search_known_pages`. Для работы нужен пользовательский env `NOTION_API_KEY` или `NOTION_TOKEN` от Notion internal integration; интеграцию надо расшарить на корневую страницу.
- 2026-05-28: `NOTION_API_KEY` сохранён в User environment рабочего ПК для internal integration `Codex` (секрет не хранить в репо и не печатать). Проверка API подтвердила валидность токена, но корневая страница пока не расшарена с integration: Notion вернул `object_not_found` и просит share relevant pages/databases with integration `Codex`.
