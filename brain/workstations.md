...
|---|---|---|
| При `AskUserQuestion` (или любой паузе, ждущей решения пользователя) — параллельно слать `PushNotification`, чтобы пользователь не пропустил, что процесс встал | ✅ добавлено 2026-06-16 (`feedback_notify_on_ask.md`) | ⬜ не добавлено — проверить, есть ли там аналог PushNotification/Remote Control |
| ЖЁСТКОЕ правило: никогда не отключать/не трогать VPN и его killswitch, ни напрямую, ни как побочный шаг диагностики — на любом компьютере, в любом чате | ✅ добавлено 2026-06-16 (`feedback_vpn_untouchable.md`); дублируется текстуально в `brain/core-work.md` | ⬜ не добавлено — добавить такую же memory-запись + продублировать в `brain/core-home.md` |
...
| Точечные правила Windows Firewall: Allow только через VPN-интерфейс + Block остальное, для `claude.exe`/`Claude.exe` (не глобальный killswitch — не должен трогать доступ к локальному серверу/NAS) | ✅ настроено, см. `grabli.md` | ⬜ не настроено — повторить по аналогии: найти текущие пути к `claude.exe`/`Claude.exe` (`Get-Process \| ? ProcessName -match claude \| select Path`) и имя VPN-интерфейса, создать 2 правила на каждый exe (`New-NetFirewallRule -Direction Outbound -Action Allow -Program <exe> -InterfaceAlias <имя VPN-адаптера>` + такой же `-Action Block` без `-InterfaceAlias`). Требует прав администратора. |
| ГЛАВНОЕ ПРАВИЛО: никогда не отключать/не трогать VPN как побочный шаг диагностики | ✅ зафиксировано в `brain/core-work.md` + в auto-memory (`feedback_vpn_untouchable.md`) | ⬜ см. строку про это правило в разделе "Чек-лист: универсальные правила Claude (auto-memory)" выше — это универсальное правило, не привязанное к этому разделу про killswitch |
...
| `telegram` | 🟢 Работает, полный доступ (read/write) | `npx @overpod/mcp-telegram` напрямую в `.mcp.json` (старый кастомный wrapper `scripts/telegram-mcp/mcp-telegram.cmd` удалён — он искал несуществующий npm-пакет `mcp-telegram`, отсюда и была поломка). Авторизован 2026-06-16: публичный `api_id=2040` блокировал и QR-логин, и пересылку кода (`SEND_CODE_UNAVAILABLE`) — обошли через установку Telegram Desktop + конвертацию `tdata` → Telethon StringSession (`opentele`, без `tgcrypto`-компилятора — патч на чистом Python с `pycryptodome`, см. `brain/grabli.md`) → base64 urlsafe→standard → `~/.mcp-telegram/session`. Аккаунт: `240962808 / JesusMaan` (тот же, что на домашнем ПК). **2026-06-16, доп. фикс**: на этом ПК Node вообще не может создавать Unix-сокеты (`net.Server.listen(path)` → `EACCES` в любой папке, не только `.mcp-telegram` — системное ограничение окружения, не специфично для telegram). `@overpod/mcp-telegram` по умолчанию поднимает IPC-сокет (`daemon.sock`) для шаринга соединения между несколькими MCP-клиентами — без патча падал фатально. Патч в `dist/master.js` (`startOwner`): ошибка `srv.listen()` теперь не реджектит promise, а просто логирует и продолжает в режиме одного процесса без межклиентского шаринга (нам и не нужно — один Claude Code, одно соединение). **Патч живёт в npx-кэше (`%LOCALAPPDATA%/npm-cache/_npx/<hash>/node_modules/@overpod/mcp-telegram/dist/master.js`), не в репозитории — пропадёт при переустановке/очистке кэша npx, тогда патчить заново** (искать `srv.listen(sock, resolve); srv.once("error", reject);` и оборачивать в try/continue вместо reject). |
...
### MCP/инструменты (домашний ПК, обновлено 2026-06-15)
