...
| rpg-mcp (text-adventure) | `C:\Users\gor-r\.local\bin\text-adventure-handler-mcp.exe` (`uv tool install --from git+https://github.com/narrowstacks/text-adventure-handler-mcp`) | нет | ✅ добавлено 2026-07-01, перенесено с домашнего | 🟢 работает |
| spotify | `C:\Users\gor-r\AppData\Roaming\npm\spotify-mcp-server.cmd` (`npm install -g @0xbarandiaran/spotify-mcp-server`) | clientId/clientSecret в `C:\Users\gor-r\.spotify-mcp\config.json` | 🟡 установлен 2026-07-01, но config.json — плейсхолдер (`PASTE_CLIENT_ID_HERE`/`PASTE_CLIENT_SECRET_HERE`); пользователь должен вставить свои Client ID/Secret со Spotify Dashboard и запустить `npx @0xbarandiaran/spotify-mcp-server auth` вручную (OAuth в браузере) | 🟢 работает |
| powershell | `C:\Users\gor-r\ps-modules\PowerShell.MCP\1.11.0\bin\win-x64\PowerShell.MCP.Proxy.exe` (`Save-Module -Name PowerShell.MCP -Path C:\Users\gor-r\ps-modules`) | нет | ✅ добавлено 2026-07-01 | 🟢 работает (версия там 1.10.0) |
...
### pyRevit (рабочий ПК, 2026-07-01)
- Установлен вручную через официальный GUI-инсталлятор (не CLI, не голый git-клон — оба варианта не сработали, см. `brain/grabli.md` раздел "pyRevit на рабочем ПК").
- Clone `master_c`, путь **`C:\pyRevit-Master`** (короткий путь на диске, НЕ `AppData\Roaming` — там была необъяснённая проблема с загрузкой DLL, см. grabli.md).
- pyRevit CLI: `C:\Users\gor-r\AppData\Local\Programs\pyRevit CLI\bin\pyrevit.exe`. Требует .NET Desktop Runtime 8.0 (доставлен отдельно).
- Attach: Revit 2024.2, engine DEFAULT (IronPython 2712).
- Собственное расширение пользователя (вкладка «Избранное») лежит **внутри клона**, не в стандартной пользовательской папке: `C:\pyRevit-Master\extensions\VK_Tools.extension` — механизм `userextensions`/Custom Extension Directories на этой машине не работает, см. `brain/grabli.md`. Бэкап исходников — `projects/pyrevit-vk/` в этом репо.
- Подробности, вся история отладки (много неочевидных граблей) — `brain/grabli.md`, раздел "pyRevit на рабочем ПК".
...
| `powershell` | 🟢 Добавлен | `C:\Users\no-na\ps-modules\PowerShell.MCP\1.10.0\bin\win-x64\PowerShell.MCP.Proxy.exe` — в `.mcp.json` |
| `spotify` | 🟢 Работает | `@0xbarandiaran/spotify-mcp-server` (глобально npm), токены в `~/.spotify-mcp/config.json`, redirectUri `http://127.0.0.1:8888/callback` зарегистрирован в Spotify Dashboard. В `claude_desktop_config.json` — прямой путь к `.cmd`. |
