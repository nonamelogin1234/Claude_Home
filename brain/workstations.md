...
| `telegram` | 🟡 Настроен, ждёт перезапуск и вход | `npx -y mcp-telegram`, конфиги `C:\Users\no-na\.codex\config.toml` и `C:\Users\no-na\.claude\settings.json`, включён `MCP_TELEGRAM_READONLY=1`; после перезапуска сказать агенту “войти в Telegram” и пройти browser login |
