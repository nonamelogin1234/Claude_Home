...
|----------|----------|
| IP | 147.45.238.120 |
| ОС | Ubuntu 22.04.4 LTS, Timeweb Нидерланды |
| Виртуализация | KVM / QEMU |
| CPU | 1 vCPU, AMD EPYC Processor |
| RAM | 1.9 GiB, без swap |
| Диск | 30 GiB (`/dev/vda1`) |
| DNS | 8.8.8.8 статический |

Проверено на живом сервере 2026-05-26. На момент проверки: доступно около 357 MiB RAM, диск занят на 64% (19/30 GiB).
На VPS уже установлен Claude Code CLI `2.1.84`. Официальное требование Claude Code — от 4 GB RAM, поэтому текущий VPS на 1.9 GiB ниже рекомендуемого минимума для постоянной интерактивной работы.
...
| Разрешённые источники | `149.255.116.76`, `146.120.69.161` (домашние внешние IP) |

- Основной защищённый SOCKS5 `:7777` не менялся.
- Если домашний внешний IP изменится, Chrome получит `ERR_SOCKS_CONNECTION_FAILED`; добавить IP в `/etc/3proxy/chrome.cfg` и перезапустить `3proxy-chrome.service`.
- [2026-06-01] Добавлен домашний IP `146.120.69.161` в allowlist `/etc/3proxy/chrome.cfg`, сервис `3proxy-chrome.service` перезапущен и проверен.
...
- 2026-06-08: для Rutracker добавлен `rutracker.org`; после переустановки стратегии `general (ALT11).bat` `https://rutracker.org/forum/index.php` отвечает `200 OK`. `www.rutracker.org/forum/index.php` отвечает `404`, использовать основной домен без `www`.
- 2026-06-01: VK Video в Vivaldi не чинить через zapret, если сеть до VK уже живая. В кейсе на домашнем Windows-ПК причина была во встроенном Vivaldi Tracker and Ad Blocker: `ERR_BLOCKED_BY_CLIENT` на служебных VK/VK Portal запросах, из-за чего сначала не создавался видеоплеер. Исправлено исключениями в `C:\Users\no-na\AppData\Local\Vivaldi\User Data\Default\AdBlockState` для `vkvideo.ru`, `vk.com`, `login.vk.com`, `vk-portal.net`, `akashi.vk-portal.net`, `userapi.com`, `vkuserlive.net`, `vkcdnservice.com`; бэкап профиля лежит на рабочем столе в папке `vivaldi-adblock-fix-backup-*`.
