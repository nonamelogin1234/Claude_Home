...
## 🚨 МЫ СУКА В ДЕСКТОПЕ, А НЕ В ТЕРМИНАЛЕ!

**Пользователь на этом ПК работает в Claude Desktop, а не в CLI Claude Code.** Это значит:
- MCP-серверы конфигурятся в `%APPDATA%\Claude\claude_desktop_config.json` — **не в `.mcp.json`** из репозитория. Правки `.mcp.json` приложение НЕ читает вообще.
- Если MCP-сервер не подключается / "ошибка загрузки" после рестарта — первым делом смотреть `claude_desktop_config.json`, а не `.mcp.json`. Лог реальных попыток запуска — `%APPDATA%\Claude\logs\mcp-server-<name>.log`.
- Уже наступали на эти грабли несколько раз за одну сессию — потратили кучу времени, правя бесполезный файл. **Не забывать снова.**
