...
def build_volume_code(parsed):
    base = f"{PROJECT_CODE}-{parsed.section_code}"
    numbers = [v for v in (parsed.subsection_number, parsed.part_number, parsed.book_number) if v]
    if not numbers:
        return base
    return base + numbers[0] + ("." + ".".join(numbers[1:]) if len(numbers) > 1 else "")
...
def build_values(source_path, change_number="1", log=None):
    ...
    values = {
        ...
        "[Наименование_файла_УЛ]": f"{path.stem}_УЛ",
    }
